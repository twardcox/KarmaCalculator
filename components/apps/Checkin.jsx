...
class Checkin extends React.Component {
	...
	refreshLatest() {
		...
		return axios
			.get(`/api/events/${this.props.eventId}/attendees/?top=${this.props.showLast}&key=${this.props.eventId}`)
			.then((response) => {
				let records = response.data.slice(0, this.props.showLast).map((value) => {
		...
		return (
			<Page title={'Karma Calculator'}>
				...
				<Section>
					...
					<ResponsiveDialog open={this.state.isCheckin} onClose={this.handleClose}>
						<DialogErrorTitle error={this.state.saveError}>Check In</DialogErrorTitle>
						<CheckinDetailForm
				...
				{(this.state.loading || (this.state.latestAttendees && this.state.latestAttendees.length !== 0)) && (
					<RecentCheckins checkins={this.state.latestAttendees} loading={this.state.loading} />
				)}
			</Page>
		);
	}
